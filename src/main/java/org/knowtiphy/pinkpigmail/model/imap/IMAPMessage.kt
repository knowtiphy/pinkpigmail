	...
	StoredPeer<MailStorage>(id, storage), IMessage
{
	companion object
	{
		...
		val GET_ATTRIBUTES: SelectBuilder = SelectBuilder()
			.addVar("*")
			.addWhere("?id", "?p", "?o")
	}
	...
	fun initialize()
	{
		...
		initialize(storage.query(folder.account.id, GET_ATTRIBUTES.buildString()))
	}
	...
	override fun getContent(allowHTML: Boolean): IPart
	{
		...
		try
		{
			...
			val content = JenaUtils.getS(
				JenaUtils.listObjectsOfPropertyU(
					context.model,
					id, Vocabulary.HAS_CONTENT
				)
			).replace("\\\"", "\"")
	...
	override val isHTML: Boolean
		get()
		{
			...
			try
			{
				return JenaUtils.getS(
					JenaUtils.listObjectsOfPropertyU(
						context.model, id,
						Vocabulary.HAS_MIME_TYPE
					)
				) == Mime.HTML
			} finally
			{
