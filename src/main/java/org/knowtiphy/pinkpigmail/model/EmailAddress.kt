...
//            val res = fst.substring(start, end)
		...
		fun format(mailAccount : IEmailAccount, addresses : List<EmailAddress>) : String
		{
			...
			{
				builder.append(addresses[i].format(mailAccount)).append(", ")
			}
	...
	fun format(mailAccount : IEmailAccount) =
		if (mailAccount.isTrustedSender(listOf(this))) (personal ?: address) else address

	override fun equals(other : Any?) : Boolean
	{
		if (this === other) return true
		if (javaClass != other?.javaClass) return false
		return address == (other as EmailAddress).address
	}
	...
	{
		return "EmailAddress(personal=$personal, address='$address')"
	}
}