...
class FolderSettings(val viewType: String = DEFAULT_VIEW, val horizontalPositions: ObservableList<SplitPane.Divider> = makeDivider(DEFAULT_HORIZONTAL_POSITION), private val verticalPositions: ObservableList<SplitPane.Divider> = makeDivider(DEFAULT_VERTICAL_POSITION))
{
    ...
    companion object
    {
        ...
        fun read(model: Model, pref: Resource): FolderSettings
        {
            ...
            val hPos = makeDivider(JenaUtils.getD(model, pref.toString(),
                    UIVocabulary.HAS_HORIZONTAL_POSITION, DEFAULT_HORIZONTAL_POSITION))
            val vPos = makeDivider(JenaUtils.getD(model, pref.toString(),
                    UIVocabulary.HAS_VERTICAL_POSITION, DEFAULT_VERTICAL_POSITION))
